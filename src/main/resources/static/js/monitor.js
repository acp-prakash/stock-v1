const TABLE_CONTAINER_ID = 'MONITOR';
const DOWNLOAD_BUTTON_ID = 'download';
const CLEAR_BUTTON_ID = 'clear';
const DATA_URL = 'stock/monitor';
var ticker;
let MONITORTable;

function createMONITORTable() {
    MONITORTable = new Tabulator(`#${TABLE_CONTAINER_ID}`, {
        height: '78.3%',
        maxHeight: '78.3%',
...
document.getElementById(DOWNLOAD_BUTTON_ID).addEventListener('click', () => {
    MONITORTable.download('xlsx', 'MONITOR.xlsx', { sheetName: 'MONITOR' });
});

document.getElementById(CLEAR_BUTTON_ID).addEventListener('click', () => {
    MONITORTable.clearHeaderFilter();
    MONITORTable.clearFilter();    
});
...
function fetchMonitorList() {
    ...
    $.ajax({
        ...
        success: function (response) {
            MONITORTable.replaceData(response);
